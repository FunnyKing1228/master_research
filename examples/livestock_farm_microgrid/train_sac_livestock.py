...
from run_month_simulation import DEFAULT_CARBON_FACTOR_KG_PER_KWH, plot_daily_weekly_impact, plot_month, summarize


ROOT = Path(__file__).resolve().parent
DEFAULT_OUTPUT_DIR = ROOT / "results" / "sac_training"


class EpisodeLogger:
    """Small SB3 callback that records episode rewards without extra deps."""
...
def train_sac(
    ...
    model_path = output_dir / "sac_livestock_microgrid.zip"
    ...
    plot_training_curve(log_df, output_dir / "training_curve.png")
    print(f"Saved SAC model: {model_path}")
    return model


def rollout_model_month(
    model: Any,
    start_date: str,
    days: int,
    initial_soc: float,
...
def plot_training_curve(log_df: pd.DataFrame, output_path: Path) -> None:
    ...
    if len(log_df) >= 5:
        rolling = log_df["episode_reward"].rolling(5, min_periods=1).mean()
        ax.plot(log_df["episode"], rolling, color="#d62728", linewidth=2.0, label="Rolling mean (5 ep)")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a short SAC policy for the livestock microgrid env.")
...
def main() -> None:
    ...
    rollout.to_csv(output_dir / f"sac_month_rollout_{suffix}.csv", index=False)
    daily.to_csv(output_dir / f"sac_month_daily_summary_{suffix}.csv", index=False)
    plot_month(rollout, daily, output_dir / f"sac_month_simulation_{suffix}.png")
    plot_daily_weekly_impact(daily, output_dir / f"sac_month_impact_summary_{suffix}.png")

    print("SAC training and rollout finished")
    print(f"Episodes logged: {len(pd.read_csv(output_dir / 'training_log.csv'))}")
